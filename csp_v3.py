...
class CSP(search.Problem):
    ...
    def nconflicts(self, var, val, assignment):
        ...
        Sara Silva 2024/25 added 'if not self.neighbors or not var in self.neighbors.keys()'"""
        # Subclasses may implement this more efficiently
        def conflict(var2):
            return (var2 in assignment and
                    not self.constraints(var, val, var2, assignment[var2]))
        if not self.neighbors or not var in self.neighbors.keys():
            #print('No neighbors!')
            return 0
        else:
            #print('var',var,'e vizinhos',self.neighbors[var])
            #for v in self.neighbors[var]:
            #    c = conflict(v)
            #if c:
            #    print('Found conflict between',var,'=',val,'and',v,'=',assignment[v])
            return count(conflict(v) for v in self.neighbors[var])
    
    '''   
    def nconflicts(self, var, val, assignment, verbose=True):
        "Return the number of conflicts var=val has with other variables."
        # Subclasses may implement this more efficiently
        def conflict(var2):
            return (var2 in assignment and
    ...
    def restore(self, removals):
        ...
        for B, b in removals:
            self.curr_domains[B].append(b)  # <--- ATTENTION: this may change the original order of values in the domains!
...
def AC3(csp, queue=None, removals=None, verbose = False):
    ...
    Sara Silva 2024/25 - added code (commented) to support unidirectional neighbor relationship """
    if queue is None:
    ...
    while queue:
        ...
        if revise(csp, Xi, Xj, removals):
            if not csp.curr_domains[Xi]:
                return False
            for Xk in csp.neighbors[Xi]:                # <--- this works only when the neighborhood arrows are bidirectional
                if Xk != Xj and (Xk,Xi) not in queue:   # <---
            #for Xk in csp.neighbors:                                               # <--- use this instead for unidirectional
            #    if Xi in csp.neighbors[Xk] and Xk != Xj and (Xk,Xi) not in queue:  # <---                 neighborhood arrows
                    queue.append((Xk, Xi))
...
def forward_checking(csp, var, value, assignment, removals, verbose = False):
    ...
    then prune neighbor values not yet assigned inconsistent with var=value.
        Sara C. Madeira - added verbose; Sara Silva 2024/25 added 'if var in csp.neighbors'"""
    if (verbose):
...
def backtracking_search_old(csp, select_unassigned_variable=first_unassigned_variable,
...
def equal_values_constraint(A, a, B, b):
    """A constraint saying two neighboring variables must be equal in value.
    (Sara Silva 2024/25)"""
    return a == b

def contains_constraint(A, a, B, b):
    """A constraint saying the first variable (lists, sets, etc) must contain all the elements of the second.
    Except when the element of the second variable is empty. 
    (Sara Silva 2024/25)"""
    return all(x in a for x in b if x)
#    for x in b:
#        if x:
#            if not x in a:
#                return False
